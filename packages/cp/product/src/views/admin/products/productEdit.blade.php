        ...
        <div class="col-md-11 mx-auto">
            <div class="card mb-2 shadow-lg">
                <div class="card-header px-2 py-2">
                    ...
                    class="fas fa-edit text-primary"></i>&nbsp; Edit Product</h3>
                            ...
                            <div class="card card-primary card-outline">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name_en" value="{{$product->name_en ?? old('name_en')}}" class="form-control" placeholder="Name English" required>
                                        @error('name_en')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="price" value="{{$product->price ?? old('price')}}" class="form-control" placeholder="Enter price">
                                        @error('price')
                                        <span style="color:red">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="discount" value="{{$product->discount ?? old('discount')}}" class="form-control" placeholder="Enter discount">
                                    </div>


                                    <div class="form-group">
                                        <label for="unit">Unit</label>
                                        <input type="text" name="unit" value="{{$product->unit ?? old('unit')}}" class="form-control" placeholder="Unit (e.g. KG, Pc etc)">
                                        @error('unit')
                                        <span style="color:red">{{ $message }}</span>
                                        @enderror
                                    </div>
                            ...
                            <div class="card card-primary card-outline">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <textarea name="excerpt_en" id="excerpt_en" class="form-control" rows="2" placeholder="Excerpt English">{{$product->excerpt_en ?? old('excerpt_en')}}</textarea>
                                    </div>

                                    <div class="form-group">
                                        <label for="excerpt_bn">Excerpt (বাংলা)</label>
                                        <textarea name="excerpt_bn" id="excerpt_bn" class="form-control" rows="2" placeholder="Excerpt (বাংলা)">{{$product->excerpt_bn ?? old('excerpt_bn')}}</textarea>
                                    </div>
                                    
                                    <div class="form-group">
                                        <label for="description_en">Description English</label>
                                        <textarea name="description_en" id="summernote" class="form-control summernote" rows="5" placeholder=" Description English">{{$product->description_en ?? old('description_en')}}</textarea>
                                    </div>


                                    <div class="form-group">
                                        <label for="description_bn">Description (বাংলা)</label>
                                        <textarea name="description_bn" id="summernote" class="form-control summernote" rows="4" placeholder=" Description (বাংলা)">{{$product->description_bn ?? old('description_bn')}}</textarea>
                                    </div>


                                    <div class="form-group">
                                        <label class="mr-3"><input type="checkbox"  name="active" checked {{  $product->active == 1 ? 'checked' : '' }}> Active</label>
                                    </div>

                                    <div class="form-group">
                                        <div class="checkbox">
                                        <label>
                                            <input type="checkbox"  name="editor" checked {{  $product->editor == 1 ? 'checked' : '' }}> Editor
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <div class="subcat-area">
                                                    <div class="custom-control custom-radio  bg-light rounded-lg mb-1">
                                                        <input type="radio" id="customSubCheckId_{{ $sub->id }}" name="subcategories_{{ $cat->id }}[]" class="custom-control-input toggle-subcategory-radio"  value="{{$sub->id}}"
                                                         {{ in_array($sub->id,$product->subcategories()->pluck('product_subcategory_id')->toArray()) ? 'checked' : " "}}
                                                        >
